  ...
  if (message.content.startsWith(`${prefix}search`)) {
    if (message.member.voice.channel) {
      const args1 = message.content.slice(prefix.length).trim().split(/ +/g);
            ...
            const args3 = args2.slice('search'.length)
    if (!args3[0]) return message.channel.send({ content: `${message.author}, Please enter a valid song name. ❌` });
        ...
        embed.setColor('BLUE');
        embed.setTitle(`Searched Music: ${args1.join(' ')}`);
